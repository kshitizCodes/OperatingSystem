...
void sort_by_at(int *pid, int *at, int *bt, int *remt, int n) {
    int i, j, temp, min;
    for (i = 0; i < n; i++) {
        min = i;
        for (j = i + 1; j < n; j++) {
    ...
    while (last > 0){
        if (active > 0){
            ...
            if (remt[0] <= 0){
                ...
                time += qt;
                active = check_active(at, time, last);
            } else { // We have moved the completed process to the end of the array and reduced the number of processes we monitor by 1
                time += qt;
                active = check_active(at, time, last);
